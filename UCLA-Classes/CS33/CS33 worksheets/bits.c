 ...
 * CS:APP Data Lab 
 * 
 * George Owen -- UID: 405196467
 *
 * On a couple functions in this lab (namely the first two),
 * I brainstormed concurrently with Shreya Chatterjee
 * 
...
int bitMask(int highbit, int lowbit) {
  //generates 2 masks, one using lowbit and one using highbit, then combines them into a single mask
  //given a single byte, highbit of 5, and lowbit of 3:
  //highMask is generated off of 5, and looks like this: 11000000. 
  //It's then inverted to 00111111
  //lowMask is set to     11111000
  //compare using &, it's 00111000, or 0x38
  int maxMask = ~0;
  int highMask = maxMask << highbit << 1; //have to shift one additional spot since we index from zero - originally I tried << (highbit + 1) but this doesn't work
  int lowMask = maxMask << lowbit;
  return ~highMask & lowMask; //inverting and comparing here
}
/* 
 ...
 *
 * originally when I wrote this, I simply added 1 then shifted right. 
 * Using addition this way makes more sense to me, but causes overflow for negative numbers.
 * this was the only implementation I could think of that works with negatives, adding a special case to handle them
 */
 int shift1 = x >> 1; 
 ...
 int bigbitpos = finalbits & ((~finalbits >> 1) ^ (1 << 31)); //when the number is negative, ~finalbits >> 1 will be 0x00000000
